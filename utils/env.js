export function SHEET_ID() {
	return "1qdlG4Hi9yhHdGRJZqTucQnNwE_DyJ9_kZIzua-LcVps";
}

export function DATA_PASSWORD() {
	return "eugenia";
}

export function CLIENT_EMAIL() {
	return "totemakvo@totemakvo.iam.gserviceaccount.com";
}

export function PRIVATE_KEY() {
	return "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n";
}
